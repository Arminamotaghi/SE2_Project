...
\caption{نمای \lr{Timeline} پروژه و توزیع وظایف در \lr{Jira}}
\end{figure}
...
با توجه به ماهیت ترافیک بالا در پلتفرم‌های فروش بلیت و خطر بروز خطای فروش مضاعف \lr{Overbooking} استراتژی‌های تست زیر در نظر گرفته شده‌اند.
...
\item \lr{Mutation Testing} به عنوان یک استراتژی پیشرفته پیشنهاد می‌شود که با ایجاد تغییرات عمدی در کد هسته رزرواسیون، از توانایی تست‌های \lr{Unit Tests} در کشف خطاهای منطقی اطمینان حاصل می‌کند و پوشش کد \lr{Code Coverage} را در ماژول‌های تراکنشی می‌سنجد.
...
همچنین در طراحی معماری، استقرار سیستم بر روی کلاستر \lr{Kubernetes} پیشنهاد شده است تا از مقیاس‌پذیری افقی خودکار \lr{Horizontal Auto-scaling} و استقرار بدون قطعی \lr{Zero-downtime Canary Upgrades} پشتیبانی شود. برای پایش وضعیت سلامت سیستم، استفاده از \lr{Prometheus} و \lr{Grafana} به عنوان راهکار پیشنهادی در نظر گرفته شده است.
...
\item \lr{Inter-Service Network Partitioning} در این سناریو ارتباط شبکه‌ای بین درگاه ورودی \lr{API Gateway} و موتورهای داخلی قطع می‌شود. برای رفع این مشکل استفاده از الگوی قطع‌کننده مدار \lr{Circuit Breaker} پیشنهاد می‌شود تا ضمن جلوگیری از انباشت درخواست‌های ناموفق سیستم به صورت خودکار پس از بازیابی شبکه عملیات را از سر بگیرد.\end{description}

\section{طراحی و تقسیم‌بندی میکروسرویس‌ها}

سیستم فروش بلیت ما بر اساس اصول \lr{Domain-Driven Design} به پنج دامنه‌ی مستقل تقسیم شده است. هر دامنه یک مسئولیت مشخص دارد و از طریق \lr{REST API} یا پیام‌رسان \lr{RabbitMQ} با سایر دامنه‌ها ارتباط برقرار می‌کند. این تقسیم‌بندی باعث کاهش وابستگی و امکان \lr{independent scaling} می‌شود.

سرویس هویت و دسترسی مسئول \lr{registration}، \lr{login} و صدور \lr{JWT token} است. کنترل دسترسی مبتنی بر نقش در این سرویس پیاده‌سازی شده و کاربران را به دو گروه \lr{Admin} و \lr{Customer} تقسیم می‌کند. برای امنیت بیشتر، \lr{token} در قالب \lr{httpOnly Cookie} ذخیره می‌شود تا در برابر حملات \lr{XSS} مقاوم باشد.

سرویس کاتالوگ و رویداد اطلاعات سالن‌ها، رویدادها و صندلی‌ها را نگهداری می‌کند. ساختار هر صندلی شامل \lr{section}، \lr{row} و \lr{seat number} است و وضعیت آن در یکی از سه حالت \lr{Available}، \lr{Locked} یا \lr{Booked} قرار دارد.

سرویس رزرو و موجودی هسته‌ی اصلی سیستم است و مسئولیت \lr{concurrency control} را بر عهده دارد. برای جلوگیری از \lr{double-booking}، از دستور اتمیک \lr{SET NX} در \lr{Redis} استفاده می‌شود. هر قفل دارای یک \lr{TTL} است تا در صورت رها شدن، صندلی به‌طور خودکار آزاد شود.

سرویس پرداخت و تسویه درخواست پرداخت را دریافت کرده و پس از تأیید مالکیت قفل، یک پیام موفقیت را به صف \lr{RabbitMQ} ارسال می‌کند. جداسازی این سرویس از عملیات ذخیره‌سازی، باعث \lr{fault tolerance} و پاسخ‌دهی سریع به کاربر می‌شود.

سرویس پردازش و اعلان به‌صورت یک \lr{worker} مستقل عمل می‌کند و پیام‌های پرداخت را از صف دریافت می‌کند. سپس وضعیت صندلی را در پایگاه‌داده به \lr{Booked} تغییر داده و قفل مربوطه را آزاد می‌کند. این معماری \lr{asynchronous} تضمین می‌کند که حتی در صورت قطعی موقت پایگاه‌داده، هیچ پیامی از دست نرود.
